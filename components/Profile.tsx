...
import { analyzeProfileImprovements, generateProfileSummary } from '../services/geminiService';
import { Sparkles, Edit2, AlertCircle, CheckCircle, Save, X, Brain, Download, Github, Linkedin, Twitter, Globe, Lock, Eye, EyeOff, Search, FileText } from 'lucide-react';
...
const Profile: React.FC<ProfileProps> = ({ user, onNavigateToDirectory }) => {
  // Analysis State
  ...
  const exportCSV = () => {
      // Create CSV content
      const headers = ['Name', 'Role', 'Email', 'Department', 'Company', 'Job Title', 'Skills', 'Bio', 'LinkedIn', 'GitHub'];
  ...
  const exportPDF = () => {
    ...
    // Socials
    if (formData.socialLinks && Object.values(formData.socialLinks).some(Boolean)) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(14);
        doc.setTextColor(0);
        doc.text("Social Links", margin, yPos);
        yPos += 8;
         ...
         {summary ? (
           ...
           <div className="text-center py-8 bg-slate-50 rounded-xl border border-dashed border-slate-200">
             <p className="text-slate-500 text-sm">
               Use Gemini AI to instantly create a professional summary based on your profile details.
             </p>
           </div>
         )}
      </div>
