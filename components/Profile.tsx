...
const Profile: React.FC<ProfileProps> = ({ user, onNavigateToDirectory, onUpdateUser, isReadOnly = false, onBack }) => {
  ...
  const exportPDF = () => {
    ...
    const subHeader = `${formData.jobTitle || formData.role} | ${formData.company || 'Alumnus'}`;
    doc.text(subHeader, margin, 30);
    
    // Body
    doc.setTextColor(30, 41, 59); // slate-800
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Professional Summary", margin, 55);
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    const summaryText = summary || formData.bio || "No summary available.";
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(formData.skills.join(" • "), margin, y);
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`${formData.department} | Class of ${formData.graduationYear}`, margin, y);
    
    // Footer
    doc.setFontSize(8);
    doc.setTextColor(148, 163, 184); // slate-400
    doc.text("Generated by AlumNexus Platform", 105, 285, { align: "center" });
    
    doc.save(`AlumNexus_Profile_${formData.name.replace(/\s+/g, '_')}.pdf`);
  };

  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
      {isReadOnly && onBack && (
          <button onClick={onBack} className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 mb-2 font-medium transition-colors">
                     ...
                     {isEditing ? (
                         <>
                             <div className="flex items-center gap-2">
                                 ...
                                 <button onClick={() => handleSync('LinkedIn', formData.socialLinks?.linkedin || '')} className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded transition-colors">
                         ...
                         <div className="flex gap-4">
                             {formData.socialLinks?.linkedin && <a href={`https://${formData.socialLinks.linkedin}`} target="_blank" rel="noreferrer" className="text-slate-500 hover:text-indigo-600 transition-colors"><Linkedin className="w-5 h-5" /></a>}
                             {formData.socialLinks?.github && <a href={`https://${formData.socialLinks.github}`} target="_blank" rel="noreferrer" className="text-slate-500 hover:text-slate-900 transition-colors"><Github className="w-5 h-5" /></a>}
                         </div>
                     )}
                 </div>
             </div>
                ...
                {isEditing ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                      {formData.skills.map((skill, index) => (
                          <span key={index} className="px-3 py-1 bg-slate-100 text-slate-700 rounded-full text-sm hover:bg-slate-200 transition-colors cursor-default">{skill}</span>
                      ))}
                  </div>
                )}
            </div>
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
         <div className="flex justify-between items-center mb-4">
           ...
           {!isReadOnly && (
             <button onClick={handleGenerateSummary} disabled={generatingSummary} className="text-sm text-violet-600 font-medium hover:text-violet-700 disabled:opacity-50 transition-colors">
                {generatingSummary ? 'Generating...' : 'Generate with AI'}
             </button>
           )}
         </div>
         {summary ? (
           <div className="p-4 bg-violet-50 rounded-xl border border-violet-100 text-slate-700 leading-relaxed animate-fade-in"><p>{summary}</p></div>
         ) : (
